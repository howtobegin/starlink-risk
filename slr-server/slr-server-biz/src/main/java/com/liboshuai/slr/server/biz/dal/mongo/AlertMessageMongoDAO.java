...
public class AlertMessageMongoDAO {
     ...
     * @return 分页结果
     */
    public PageResult<AlertMessageDO> selectPage(AlertMessagePageReqVO alertMessagePageReqVO) {
        Query query = new Query();

        List<Criteria> criteriaList = new ArrayList<>();

        String channel = alertMessagePageReqVO.getChannel();
        if (channel != null && !channel.isEmpty()) {
            criteriaList.add(Criteria.where("channel").is(channel));
        }
        String ruleCode = alertMessagePageReqVO.getRuleCode();
        if (ruleCode != null && !ruleCode.isEmpty()) {
            criteriaList.add(Criteria.where("ruleCode").is(ruleCode));
        }
        String alertMessage = alertMessagePageReqVO.getAlertMessage();
        if (alertMessage != null && !alertMessage.isEmpty()) {
            // 使用正则表达式进行模糊查询，'i'表示不区分大小写
            criteriaList.add(Criteria.where("alertMessage").regex(alertMessage, "i"));
        }
        LocalDateTime alertTimeStart = alertMessagePageReqVO.getAlertTimeStart();
        LocalDateTime alertTimeEnd = alertMessagePageReqVO.getAlertTimeEnd();
        ...
        int pageNo = alertMessagePageReqVO.getPageNo();
        int pageSize = alertMessagePageReqVO.getPageSize();
        // 设置分页参数
        // 页码从0开始，需要减1
