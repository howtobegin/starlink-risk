...
public class CoreFunction extends KeyedBroadcastProcessFunction<String, FlinkEventDTO, MysqlCdcDTO, FlinkResultDTO> implements CheckpointedFunction {
    ...
    public void processElement(FlinkEventDTO flinkEventDTO,
        ...
        log.debug("processElement - FlinkEventDTO:{}, key:{}, timestamp:{}", flinkEventDTO, currentKey, timestamp);
...
//                log.debug("processElement - !oldRuleListState.contains(ruleCode), key:{}, timestamp:{}", currentKey, timestamp);
...
//                log.debug("processElement - else, key:{}, timestamp:{}", currentKey, timestamp);
                // 否则直接处理当前一条事件数据即可
        ...
        if (isOnTime) {
            ...
            log.debug("processElement - 注册了定时器, fireTime:{}, key:{}, timestamp:{}", fireTime, currentKey, ctx.currentProcessingTime());
        }
    }
    ...
    private void clearOldState(StateDTO stateDTO) throws Exception {
        ...
        nextEndTimestampState = runtimeContext.getMapState(ProcessorOneStateDesc.getNextEndTimestampStateDesc(ruleCode, ruleVersion));
...
//        logState("之后");
    ...
    private void logState(String status) throws Exception {
        ...
        Map<String, Long> nextEndTimestampMap = new HashMap<>();
        Iterator<Map.Entry<String, Long>> nextEndTimestampMapIterator = nextEndTimestampState.iterator();
        while (nextEndTimestampMapIterator.hasNext()) {
            Map.Entry<String, Long> next = nextEndTimestampMapIterator.next();
            nextEndTimestampMap.put(next.getKey(), next.getValue());
        }
        ...
        log.debug("nextEndTimestampMap: {}", JsonUtils.toJsonString(nextEndTimestampMap));
...
//        Processor processor = mockProcessor(runtimeContext, null, ruleInfoDTO);
    ...
    public void onTimer(long timestamp,
                        ...
                        Collector<FlinkResultDTO> out) throws Exception {
        log.debug("onTimer - 执行了定时器, key:{}, timestamp:{}", ctx.getCurrentKey(), timestamp);
        ...
        if (hasPendingTimers) {
            log.debug("onTimer - 注册了定时器, key:{}, timestamp:{}", ctx.getCurrentKey(), timestamp);
